...
def update_driver(driver_id, name):
   ...
   cur.execute(
      "update driver set name=%s where driver_id=%s",
      (name, driver_id)
   )
   conn.commit()
   cur.close()
...
def update_vehicle(vehicle_id, model):
    conn = get_connection()
    cur = conn.cursor()

    cur.execute("""
                update vehicle set model=%s where vehicle_id=%s""",
                (model, vehicle_id))
...
def delete_vehicle(vehicle_id):
    ...
    cur.execute("""
                delete from vehicle where vehicle_id=%s""", (vehicle_id,))
...
def update_route(route_id, service_zone):
    ...
    cur.execute("""
                update route set service_zone=%s where route_id=%s""",
...
def delete_route(route_id):
    ...
    cur.execute("""
                delete from route where route_id=%s""", (route_id,))
...
def update_package(package_id, description):
    conn = get_connection()
    cur = conn.cursor()

    cur.execute("""
                update package set description=%s where package_id=%s""",
...
def delete_package(package_id):
    ...
    cur.execute("""
                delete from package where package_id=%s""", (package_id,))
